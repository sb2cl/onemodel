...
class OneModelHighlighter(QSyntaxHighlighter):
    """Syntax highlighter for the OneModel language.
    """
    # OneModel keywords

    keywords = [
        'species', 'parameter', 'reaction', 'rule', 'end', 'function', 'model'
    ]
    ...
    def __init__(self, document):
        ...
        rules += [(r'\b%s\b' % w, 0, STYLES['keyword'])
                  for w in OneModelHighlighter.keywords]
        rules += [(r'%s' % o, 0, STYLES['operator'])
                  for o in OneModelHighlighter.operators]
        rules += [(r'%s' % b, 0, STYLES['brace'])
                  for b in OneModelHighlighter.braces]
