...
class RuleRate(Value):
    ...
    def add_value_to_model(self, name, model):
        ...
        check(r, f"create rate rule {name}")

        check(r.setIdAttribute(name), f"set rate rule id {name}")

        check(r.setVariable(variable), f"set variable on rate rule {name}")

        check(r.setMath(math_ast), f"set math on rate rule {name}")
