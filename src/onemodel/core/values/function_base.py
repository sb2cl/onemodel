...
class FunctionBase(Value):
    ...
    def generate_execution_context(self, calling_context):
        ...
        evaluated.
        """
        execution_context = Context(f"{self.name}", calling_context)
    ...
    def check_arguments(self, arg_names, args):
        ...
        if len(args) > len(arg_names):
            raise Exception(f"{len(args) - len(arg_names)} too many arguments passed to {self}")

        if len(args) < len(arg_names):
            raise Exception(f"{len(args) - len(arg_names)} too few arguments passed to {self}")
