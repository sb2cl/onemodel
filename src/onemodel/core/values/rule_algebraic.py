...
class RuleAlgebraic(Value):
    ...
    def add_value_to_model(self, name, model):
        ...
        check(r, f"create algebraic rule {name}")

        check(r.setIdAttribute(name), f"set algebraic rule id {name}")

        check(r.setMath(math_ast), f"set math on algebraic rule {name}")
