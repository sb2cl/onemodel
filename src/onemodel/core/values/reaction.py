...
class Reaction(Value):
    ...
    def add_value_to_model(self, name, model):
        ...
        check(r, f"create reaction {name}")

        check(r.setId(name), f"set reaction id {name}")

        check(r.setReversible(self.reversible), "set reaction reversibility flag")
        ...
        for item in self.reactants:
            ...
            check(species_ref.setSpecies(item_name), f"assign reactant species {item_name}")

            check(species_ref.setConstant(True), f'set "constant" on species {item_name}')
        ...
        for item in self.products:
            ...
            check(species_ref.setSpecies(item_name), "assign product species")
        ...
        for item in names_modifier:
            ...
            check(modifier_ref, "create modifier")

            check(modifier_ref.setSpecies(item), "assign modifier species")
